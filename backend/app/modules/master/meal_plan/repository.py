...
class MealPlanRepository(SQLAlchemyBaseRepository[MealPlan]):
    ...
    def find_by_code(self, code: str) -> MealPlan | None:
        return self.model_class.query.filter_by(code=code.strip().upper()).first()

    def find_by_code_excluding(self, code: str, exclude_id: uuid.UUID) -> MealPlan | None:
