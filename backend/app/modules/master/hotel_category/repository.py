...
class HotelCategoryRepository(SQLAlchemyBaseRepository[HotelCategory]):
    searchable_fields = ["name", "code"]
    filterable_fields = ["is_active"]

    def __init__(self):
        super().__init__(HotelCategory)

    def find_by_code(self, code: str) -> HotelCategory | None:
        return self.model_class.query.filter_by(code=code.strip().upper()).first()

    def find_by_code_excluding(self, code: str, exclude_id: uuid.UUID) -> HotelCategory | None:
