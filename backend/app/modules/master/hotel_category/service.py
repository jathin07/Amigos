...
from .repository import HotelCategoryRepository
from .models import HotelCategory

class HotelCategoryService(BaseService):
    def __init__(self):
        self.repository = HotelCategoryRepository()
    ...
    def _parse_uuid(self, raw_id):
        ...
        try:
            ...
            raise NotFoundException("HotelCategory not found.", code="ERR_NOT_FOUND")

    def create(self, data: dict) -> HotelCategory:
        code = data["code"].upper()
        if self.repository.find_by_code(code):
            raise BusinessException(f"Code '{code}' already exists.", code="ERR_DUPLICATE_CODE")
        entity = HotelCategory(
    ...
    def update(self, entity_id: str, data: dict) -> HotelCategory:
    ...
    def get(self, entity_id: str) -> HotelCategory:
        uid = self._parse_uuid(entity_id)
        entity = self.repository.get(uid)
        if not entity:
            raise NotFoundException("HotelCategory not found.", code="ERR_NOT_FOUND")
