...
class CreateCountryRequestSchema(Schema):
    name          = fields.String(required=True,  validate=validate.Length(min=1, max=100))
    code          = fields.String(required=True,  validate=validate.Length(min=1, max=10))
    phone_code    = fields.String(required=True,  validate=validate.Length(min=1, max=10))
    ...
    def validate_phone_code(self, value, **kwargs):
        if not re.match(r"^\+\d+$", value.strip()):
            raise ValidationError("Phone code must be in format +XX or +XXX (e.g. +91).")
        return value.strip()


class UpdateCountryRequestSchema(Schema):
    name          = fields.String(validate=validate.Length(min=1, max=100))
    code          = fields.String(validate=validate.Length(min=1, max=10))
    phone_code    = fields.String(validate=validate.Length(min=1, max=10))
...
class CountrySummaryResponseSchema(Schema):
    id        = fields.UUID()
    name      = fields.String()
    code      = fields.String()
    is_active = fields.Boolean()
...
class CountryDetailResponseSchema(CountrySummaryResponseSchema):
...
class CountryLookupResponseSchema(Schema):
    id   = fields.UUID()
    name = fields.String()
    code = fields.String()
