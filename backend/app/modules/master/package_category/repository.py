...
class PackageCategoryRepository(SQLAlchemyBaseRepository[PackageCategory]):
    searchable_fields = ["name", "code"]
    filterable_fields = ["is_active"]

    def __init__(self):
        super().__init__(PackageCategory)

    def find_by_code(self, code: str) -> PackageCategory | None:
        return self.model_class.query.filter_by(code=code.strip().upper()).first()

    def find_by_code_excluding(self, code: str, exclude_id: uuid.UUID) -> PackageCategory | None:
